...
void extraTests() {
    PairingPQ<int> pq;
        ...
        // Insert a node and store a pointer to it
        PairingPQ<int>::Node* node = pq.addNode(25);
        
        // Call updateElt on the node multiple times
        pq.updateElt(node, 15);
        pq.updateElt(node, 5);
        pq.updateElt(node, 3);
        
        // Call pop() on the queue multiple times
        pq.pop();
        pq.pop();
        pq.pop();
    pq.pop();
        
        // Attempt to delete the node manually, causing a double free/double delete error
        //delete node;
}
...
int main() {
    ...
    //IMPORTANT: THIS STATIS CAST WAS MANUALLY ADDED TO HELP IT COMPILE
